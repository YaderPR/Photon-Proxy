...
impl ProxyAdapter for LinuxTproxyAdapter {
    async fn start(&mut self, app_handle: tauri::AppHandle) -> Result<()> {
        ...
        if let Some(listener) = self.tcp_listener.take() {
            ...
            tokio::spawn(async move {
                Self::tcp_accept_loop(listener, port, upstream_cfg, handle, shutdown_rx).await;
            });
        }
        ...
        if let Some(socket) = self.udp_socket.take() {
            ...
            let shutdown_rx = self.shutdown_tx.subscribe();
            tokio::spawn(async move {
                if let Err(e) = Self::udp_recv_loop(socket, upstream_cfg, handle, shutdown_rx).await
                {
    ...
    async fn stop(&mut self) -> Result<()> {
        ...
        self.shutdown_tx.send(true).ok(); // Signal all background loops to stop
...
impl LinuxTproxyAdapter {
        ...
        loop {
            tokio::select! {
                _ = shutdown_rx.changed() => {
                    info!("TCP accept loop shutting down gracefully");
                    break;
                }
                result = listener.accept() => {
                    match result {
                        Ok((mut client_stream, src_addr)) => {
                            // Enable TCP_NODELAY to avoid Nagle latency on TLS handshakes
                            ...
                            tokio::spawn(async move {
                                ...
                                .await
                                {
                                    log::debug!("TCP connection ended for {}: {}", target_addr, e);
                                    Self::emit_log(
                                        &handle_clone,
                                        "ERROR",
                                        "TCP",
    ...
    async fn udp_recv_loop(
        ...
        let sessions: Arc<RwLock<HashMap<(SocketAddr, SocketAddr), Arc<UdpSession>>>> =
        ...
        loop {
            tokio::select! {
                _ = shutdown_rx.changed() => {
                    info!("UDP recv loop shutting down gracefully");
                    break;
                }
                r = socket.readable() => { r?; }
            }
            ...
            if let Ok(Some((n, client_addr, dst_addr))) = result {
                ...
                let session_key = (client_addr, dst_addr);

                // Read-first pattern: avoid write lock contention under high UDP traffic
                let session = {
                ...
                let session = if let Some(s) = session {
                    ...
                    if let Some(s) = session_map.get(&session_key) {
                        s.clone()
                    } else {
                        let s = Arc::new(
                            UdpSession::new(
                        ...
                        session_map.insert(session_key, s.clone());
    ...
    async fn handle_tcp_connection(
        ...
        // 1. Establish connection to the proxy/relay with timeout
        let mut server_stream = tokio::time::timeout(
            Duration::from_secs(10),
            Self::connect_upstream(target_addr, upstream),
        )
        .await
        .map_err(|_| anyhow::anyhow!("Upstream connection timed out after 10s"))??;

        // Enable TCP_NODELAY on upstream socket too
    ...
    async fn connect_upstream(
        ...
        match upstream {
            ...
            UpstreamProtocol::Socks5 {
                ...
                Ok(Socks5Client.connect(target_addr, server, u, p).await?)
            }
            UpstreamProtocol::HttpProxy {
                ...
                let u = username.as_deref();
                let p = password.as_deref();
                Ok(HttpProxyClient.connect(target_addr, server, u, p).await?)
            }
        }
    }
    ...
    async fn resolve_dns_via_proxy(query: &[u8], upstream: &UpstreamProtocol) -> Result<Vec<u8>> {
        ...
        // For HTTP Proxy: CONNECT to port 53 is rejected by most proxies.
        // Use a direct marked socket instead (DNS bypass).
        let mut stream = match upstream {
            UpstreamProtocol::Direct | UpstreamProtocol::HttpProxy { .. } => {
        ...
        // DNS over TCP: Send [Length: u16, Query: ...]
        ...
        // Read response [Length: u16, Response: ...]
